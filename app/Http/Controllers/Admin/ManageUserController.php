...
class ManageUserController extends Controller
{
    ...
    public function impersonate($id)
    {
        ...
        return redirect()->route('home'); // or user dashboard
    }
    ...
    public function stopImpersonate()
    {
        if (session()->has('impersonate')) {
            ...
            // Optionally, log back in as admin or redirect to admin login
    ...
    public function store(Request $request)
    {
        ...
        $validated['plain_password'] = $generatedPassword;

        // Generate usertag: HV + 7 random digits
        $generatedUsertag = 'HV' . str_pad(strval(random_int(0, 9999999)), 7, '0', STR_PAD_LEFT);
        $validated['usertag'] = $generatedUsertag;

        // Save user
        $user = User::create($validated);

        // Pass generated password and usertag to the view via session
        return redirect()->route('admin.manage.users')
            ->with('status', 'User created successfully!')
            ->with('generated_password', $generatedPassword)
            ->with('generated_usertag', $generatedUsertag);
    }
